...
public class FirstNonDuplicatedValue {
    public static Integer findFirstNonDuplicate(int[] array) {
        ...
        // Second pass: Find the first non-duplicated element
        for (int num : array) {
            if (frequencyMap.get(num) == 1) {
                return num;  // Return the first non-duplicated value
            }
        }
        
        // Return null if no non-duplicated element is found
        return null;
    }

    public static void main(String[] args) {
        int[] array = {4, 5, 1, 2, 5, 2, 4, 7};
        Integer result = findFirstNonDuplicate(array);
        if (result != null) {
            System.out.println("First non-duplicated value: " + result);
        } else {
            System.out.println("No non-duplicated value found.");
        }
    }
}
